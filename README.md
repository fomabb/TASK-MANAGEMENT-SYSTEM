...
Для удобства вы можете клонировать проект и скачать файл `.env`, который содержит необходимые настройки для запуска приложения. Нужно его разместить в корне проекта. После этого вы сможете запустить приложение. Также вы можете скачать архив целиком, где расположены файлы `.env` и `docker-compose.yml`, распаковать у себя на компьютере и запустить через комндную строку. ```Windows``` `cd путь к папке`.
...
После скачивания архива или файла `.env`, запустите приложение с помощью следующей команды:
